...
47. If correct option is selected, ponits being awarded before clicking 'Reveal Correct Answer' button. Award points only after revealing correc - D answer - Done
48. Rules not opening on Audience console when Rules button is clicked on host console. To open rules on audience console, rules button on audience screen needs to be clicked. - Done
49. Sync Rules popop window open/close on host and audience console - Done
50. On host console - Do not ask for Trump card selection if both Trump cards are used by a team. Just show that trump cards are used - Done
51. On clicking lifeline, should we pause the clock till lifelife is confirmed? - Done